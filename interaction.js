...
export function pilePosition(index){
  const angle=seed(index)*Math.PI*2,radius=Math.sqrt(seed(index,1))
  return {x:.5+Math.cos(angle)*radius*.32,y:.64+Math.sin(angle)*radius*.23,angle:(seed(index,2)-.5)*58,scale:.78+seed(index,3)*.32,depth:Math.round(seed(index,4)*100)}
}
